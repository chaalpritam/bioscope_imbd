...
class Genre(models.Model):
    ...
    def __str__(self): return "%s" % self.genres

class Country(models.Model):
    country = models.CharField(max_length=60, unique=True)
    def __str__(self): return "%s" % self.country

class Stars(models.Model):
    stars = models.CharField(max_length=40, unique=True)
    def __str__(self): return "%s" % self.stars
...
class  Film(models.Model):
      ...
      def __str__(self): return "%s" % self.film_name
